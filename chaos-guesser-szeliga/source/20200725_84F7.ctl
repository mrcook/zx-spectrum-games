...
C $856A ...
C $8582 Call #R$9786 to see if target is in range ???
C $8585 ...
C $858D Load HL with the cursor position.
C $8590 Load DE with an offset into table 4.
C $8594 Test the 'UNDEAD' bit of the corresponding byte.
C $8596 If the target is an undead creature then jump to #R$8557.
C $8598 Otherwise call #R$859C.
C $859B Return.
C $859C Load A with the vale of #R$AC2E.
C $859F See if A is set.
C $85A0 If the current player is not computer-controlled then jump to #R$85A5.
...
C $85A8 Load HL with the cursor position.
C $85AB Load DE with an offset into the 4th map table.
C $85AE Point HL at the corresponding byte in the 4th map table.
C $85AF See if the current creature has been disbelieved [I think].
C $85B1 If the creature has been marked as disbelieved then the spell must fail so jump to #R$85CA.
C $85B3 Otherwise load HL with the cursor position.
C $85B6 Load D with the object at the cursor.
...
C $85C0 Call #R$904B  to load A with a random number that is 0 to 9.
